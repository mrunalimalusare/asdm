...
class LoginController extends CI_Controller {
	...
	public function index(){
		...
		$data['title'] = "Login | Cdac";
		$this->load->view('login-view/login-page',$data);
		
	}

	public function register(){
		$data['title'] = "Register | Cdac";
		$this->load->view('login-view/register',$data);
	}

	public function forgetpassword(){
		$data['title'] = "Forget Password | Cdac";
		$this->load->view('login-view/forgetpassword',$data);
	}
	...
	public function register_service(){
		...
		$result = $this->LoginModel->register_user($fullname,$email,$password);
		if($result==TRUE){
			echo 'success';
		}else{
			echo 'error';
	...
	public function send_email($htmlContent){
		$this->load->library('email');
		$config = array(
			'protocol'=>'smtp',
			'smtp_host'=>'ssl://smtp.googlemail.com',
			'smtp_port'=>465,
			'smtp_user'=>'tica.cdac2020@gmail.com',
			'smtp_pass'=>'Delta@111',
			'mailtype'=>'html',
			'charset'=> 'utf-8'
		);
		...
		$this->email->to('chiragdraul@gmail.com');
		$this->email->from('tica.cdac2020@gmail.com','TICA-CDAC');
		$this->email->subject('Forget Passwod');
		$this->email->message($htmlContent);
		$this->email->send();
	}

	public function frogetpass_service(){
		$fullname = $this->input->post('full_name');
		$email = $this->input->post('email');
		$result = $this->LoginModel->forgetpassword($fullname,$email);
		if($result!=''){		
																					...
																					<tbody>
																						...
																						<tr>
																							<td class="esd-block-text es-p40r es-p40l" align="left">
																								<p style="text-align: center;">HI, <b>'.$result[0]['full_name'].'</b></p>
																							</td>
																						</tr>
																						<tr>
																						...
																						<tr>
																							<td class="esd-block-text es-p25t es-p40r es-p40l" align="center">
																								<p>If did not make this request, just ignore this email. Otherwise, your username = <b>'.$result[0]['email'].'</b> and password = <b>'.$result[0]['password'].'</b></p>
																							</td>
																						</tr>
	...
	public function login_service(){
		$email = $this->input->post('email');
		$password = $this->input->post('password');
		$result = $this->LoginModel->logindata($email,$password);
		...
		if($result!=''){
			if($result[0]['status']==200){
				...
				if($user_role=='admin'){
					$result['redirect_url'] = base_url('admin');
				}
				if($user_role=='user'){
					$result['redirect_url'] = base_url('user');
				}
				$result['status'] = "success";
			}
			if($result[0]['status']==500){
				$result['status'] = "block";
				$result['msg'] = "Your Account is Blocked...";
			}
			...
			$result['status'] = "error";
			$result['msg'] = "Please Check Username and Password...";
		}
